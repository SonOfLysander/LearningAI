		...
		while (true) {
			...
			for (int x = pool.size() - 1; x >= 0; x -= 2) {
				...
				if (n1.total == target && n1.isValid()) {
					System.out.println("Generations: " + gen + "  Solution: "
							+ n1.decodeChromo());
					return;
				}
	...
	private static class Chomosone {
		...
		public Chomosone(int target) {
			...
			for (int y = 0; y < chromoLen; y++) {
				...
				String binString = Integer.toBinaryString(rand
						.nextInt(ltable.length));
		...
		public final boolean isValid() {
			...
			if (!Character
					.isDigit(decodedString.charAt(decodedString.length() - 1)))
				return false;
